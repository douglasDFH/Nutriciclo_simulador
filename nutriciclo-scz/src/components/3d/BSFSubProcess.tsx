...
const LABEL_STYLE = {
  ...
  borderRadius: 6, border: '1px solid #a3e635',
}
...
export function BSFSubProcess() {
  ...
  useFrame((_, delta) => {
    if (larvaeRef.current && bioActive)   larvaeRef.current.rotation.y  += delta * 0.2
    if (bsfDryRef.current && dryActive)   bsfDryRef.current.rotation.z  += delta * 0.4
    if (bsfMillRef.current && millActive) bsfMillRef.current.rotation.y += delta * 3
  })
  ...
  function toggle(id: string, e: ThreeEvent<MouseEvent>) {
    e.stopPropagation()
  ...
  return (
    <group position={[-4, 0, -18]}>
      ...
      {/* Biorreactor BSF */}
      <group position={[0, 0, 0]} onClick={(e) => toggle('bio', e)}>
        <mesh castShadow>
          <boxGeometry args={[1.4, 1.8, 1.2]} />
          <meshStandardMaterial color={bioColor} metalness={0.2} roughness={0.8}
            transparent opacity={0.85} emissive={bioActive ? bioColor : '#000'} emissiveIntensity={0.15} />
        </mesh>
        ...
        {bioActive && <pointLight position={[0, 1, 0]} intensity={0.4} color="#a3e635" distance={3} />}
        {selected === 'bio' && (
          <Html position={[0, -1.3, 0]} center>
            <div style={{ ...LABEL_STYLE, color: '#a3e635', fontSize: '10px', whiteSpace: 'nowrap', textAlign: 'center', pointerEvents: 'none', lineHeight: 1.4 }}>
              ...
              {bioActive && <span style={{ color: '#a3e635', fontWeight: 700, fontSize: '8px' }}>● CRÍA ACTIVA</span>}
            </div>
          </Html>
        )}
      </group>
      ...
      {/* Secador BSF */}
      <group position={[3.5, 0.2, 0]} onClick={(e) => toggle('dry', e)}>
        <mesh ref={bsfDryRef} castShadow rotation={[0, 0, Math.PI / 2]}>
          <cylinderGeometry args={[0.45, 0.45, 2.2, 16]} />
          <meshStandardMaterial color={dryColor} metalness={0.6} roughness={0.4}
            emissive={dryActive ? dryColor : '#000'} emissiveIntensity={0.2} />
        </mesh>
        {dryActive && <pointLight position={[0, 0, 0]} intensity={0.5} color="#fb923c" distance={3} />}
        {selected === 'dry' && (
      ...
      {/* Molino BSF */}
      <group position={[7, 0, 0]} onClick={(e) => toggle('mill', e)}>
        <mesh castShadow>
        ...
        {selected === 'mill' && (
          <Html position={[0, -0.8, 0]} center>
            <div style={{ ...LABEL_STYLE, color: '#a3e635', fontSize: '10px', whiteSpace: 'nowrap', textAlign: 'center', pointerEvents: 'none', lineHeight: 1.4 }}>
              ...
              <span style={{ color: '#a3e635', fontWeight: 700 }}>{sensors.bsfFlourRate.toFixed(1)} kg/h</span>
            </div>
          </Html>
        )}
      </group>

      {/* Badge tasa BSF (siempre visible cuando activo) */}
      {millActive && sensors.bsfFlourRate > 0 && (
        <Html position={[3.5, 2.5, 0]} center>
          <div style={{ background: 'rgba(16,42,0,0.85)', border: '1px solid #a3e635',
            borderRadius: 6, padding: '4px 10px', color: '#a3e635',
            fontSize: '11px', fontWeight: 700, whiteSpace: 'nowrap', pointerEvents: 'none' }}>
