...
const LABEL_STYLE = {
  ...
  borderRadius: 6, border: '1px solid #22c55e',
}
...
export function Phase3Equipment() {
  ...
  useFrame((_, delta) => {
    ...
    if (vibRef.current && vibActive)        vibRef.current.position.y = -0.3 + Math.sin(Date.now() * 0.05) * 0.015
  ...
  const paddleColor = paddleActive
    ? equipment.paddle_mixer.status === 'warning' ? '#facc15' : '#22c55e' : '#374151'
  const limeColor = limeActive
    ? sensors.exothermicTemp > 150 ? '#ef4444'
    : sensors.exothermicTemp > 80  ? '#f97316' : '#22c55e' : '#374151'
  ...
  return (
    <group position={[16, 0, 0]}>
      ...
      <group position={[0, 0, 0]} onClick={(e) => toggle('paddle', e)}>
        <mesh castShadow>
        ...
        {selected === 'paddle' && (
          <Html position={[0, -0.8, 0]} center>
            <div style={{ ...LABEL_STYLE, color: '#86efac', fontSize: '10px', whiteSpace: 'nowrap', textAlign: 'center', pointerEvents: 'none', lineHeight: 1.4 }}>
      ...
      <group position={[2.5, 0, 0]} onClick={(e) => toggle('lime', e)}>
        <mesh castShadow>
      ...
      {/* Vibradora de Mesa */}
      <group position={[5, -0.3, 0]} onClick={(e) => toggle('vib', e)}>
        ...
        {[[-0.6, 0], [0, 0], [0.6, 0]].map(([x, z], i) => (
          <mesh key={i} position={[x, 0.25, z]} castShadow>
            ...
            <meshStandardMaterial color={vibActive ? '#4ade80' : '#4b5563'} metalness={0.3} roughness={0.7} />
          </mesh>
        ))}
        {selected === 'vib' && (
      ...
      <group position={[7.5, -0.5, 0]} onClick={(e) => toggle('belt', e)}>
        <mesh castShadow>
      ...
      {/* Ventilación */}
      <group position={[2.5, 2.5, 0]} onClick={(e) => toggle('vent', e)}>
        <mesh castShadow>
