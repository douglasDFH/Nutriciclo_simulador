...
export function Phase2Equipment() {
  ...
  return (
    <group position={[-1, 0, 0]}>
      ...
      {/* Tanque Melaza */}
      <group position={[-3, 0, 0]} onClick={(e) => toggle('tank', e)}>
        <mesh castShadow>
          <cylinderGeometry args={[0.9, 0.6, 2.2, 20]} />
          <meshStandardMaterial color={tankColor} metalness={0.6} roughness={0.3}
            transparent opacity={0.85} emissive={tankActive ? tankColor : '#000'} emissiveIntensity={0.1} />
        </mesh>
        {selected === 'tank' && (
          <Html position={[0, -1.5, 0]} center>
            <div style={{ ...LABEL_STYLE, color: '#93c5fd', fontSize: '10px', whiteSpace: 'nowrap', textAlign: 'center', pointerEvents: 'none', lineHeight: 1.4 }}>
      ...
      {/* Bomba Peristáltica */}
      <group position={[-1.2, -0.3, 1.5]} onClick={(e) => toggle('pump', e)}>
        <mesh castShadow>
      ...
      {/* Batea de Disolución */}
      <group position={[1, 0, 1.5]} onClick={(e) => toggle('dissolv', e)}>
        <mesh castShadow>
          <boxGeometry args={[1.0, 0.8, 0.9]} />
          <meshStandardMaterial color={dissolvColor} metalness={0.4} roughness={0.5}
            transparent opacity={0.9} emissive={dissolvActive ? dissolvColor : '#000'} emissiveIntensity={0.1} />
        </mesh>
        {selected === 'dissolv' && (
      ...
      {/* Bomba Centrífuga */}
      <group position={[2.8, -0.3, 1.5]} onClick={(e) => toggle('transfer', e)}>
      ...
      <group position={[4.5, 0, 0]} onClick={(e) => toggle('mixer', e)}>
        <mesh castShadow>
          <boxGeometry args={[3.0, 1.0, 1.2]} />
          <meshStandardMaterial color={mixerColor} metalness={0.6} roughness={0.3}
            transparent opacity={0.85} emissive={mixerActive ? mixerColor : '#000'} emissiveIntensity={0.1} />
        </mesh>
        ...
        {selected === 'mixer' && (
          <Html position={[0, -0.8, 0]} center>
            <div style={{ ...LABEL_STYLE, color: '#93c5fd', fontSize: '10px', whiteSpace: 'nowrap', textAlign: 'center', pointerEvents: 'none', lineHeight: 1.4 }}>
              ...
              <span style={{ color: '#9ca3af', fontSize: '8px' }}>500–3000 kg/lote · 37 kW</span><br/>
